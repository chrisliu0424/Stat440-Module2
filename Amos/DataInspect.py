...
for i in range(1):
    model.train()
    model.cuda()
    path = "./" + str(y_train.columns[i]) + ".pth" 
    y_tensor = y_train[y_train.columns[i]]
    ...
    y_predid = pd.DataFrame(y_predid,columns=([str(y_train.columns[i])]))
    result = pd.concat([result.reset_index(drop=True), y_predid], axis=1)
    
    train_test =  model(x_train_tensor)
    train_test = train_test.detach().numpy()
    train_test = pd.DataFrame(train_test,columns=([str(y_train.columns[i])]))
